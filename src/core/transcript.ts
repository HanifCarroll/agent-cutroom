...
export async function loadTranscript(path: string): Promise<LoadedTranscript> {
  ...
  if (sourceSegments.length > 0 && segments.length !== sourceSegments.length) {
    warnings.push(
      `Imported ${segments.length} of ${sourceSegments.length} transcript segments; skipped segments were missing text/start/end.`,
    );
  }
  if (sourceSegments.length === 0 && untimedText) {
    warnings.push(
